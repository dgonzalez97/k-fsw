...
| PARAM CSP adapter | Optional libparam-compatible server/client/cache | Two-node native remote list/get/set plus Robot errors | Holybro RF bench passed production list/get/set, valid owner callback, restore, and invalid-value retention | Fixed remote descriptor pool; no remote persistence command |
...
| K-FSW FTP | LIST/STAT/MKDIR/PUT/GET, 192-byte chunks, CRC, sandbox, atomic `.part` finalization | Protocol ztest, native transfers from 0 bytes through 8 KiB, Robot workflow | 4 KiB and 16 KiB round trips on NUCLEO UART bench | K-FSW protocol, not Internet FTP; one server worker/client workspace |
...
events, health, housekeeping, scheduling, and equipment modules should be
introduced one contract at a time. Older architecture/reference notes are
design input, not an already-approved feature list. Each new capability needs
an owner, Kconfig boundary, target composition, tests, operational semantics,
and an honest status row.
