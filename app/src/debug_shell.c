...
static int cmd_kfsw_ftp_list(const struct shell *sh, size_t argc, char **argv)
{
	uint16_t node;
...
static int cmd_kfsw_ftp_stat(const struct shell *sh, size_t argc, char **argv)
{
	uint16_t node;
	int result;
...
static int cmd_kfsw_ftp_mkdir(const struct shell *sh, size_t argc, char **argv)
{
	uint16_t node;
	int result;
...
static int cmd_kfsw_ftp_get(const struct shell *sh, size_t argc, char **argv)
{
	uint16_t node;
	int result;
...
static int cmd_kfsw_ftp_put(const struct shell *sh, size_t argc, char **argv)
{
	uint16_t node;
	int result;
...
static int create_ftp_fixture(const char *path, uint32_t size, uint32_t *crc32)
{
...
static int compare_ftp_local_files(const char *first_path, const char *second_path)
{
...
static int cmd_kfsw_ftp_generate(const struct shell *sh, size_t argc, char **argv)
{
	unsigned long size;
	uint32_t crc32;
	int parse_error = 0;
	...
	result = create_ftp_fixture(argv[1], (uint32_t)size, &crc32);
	if (result != 0) {
...
static int cmd_kfsw_ftp_verify(const struct shell *sh, size_t argc, char **argv)
{
	int result;

	ARG_UNUSED(argc);
	result = compare_ftp_local_files(argv[1], argv[2]);
	if (result != 0) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(generate, NULL, "Create deterministic local data: generate <path> <bytes>.",
		      cmd_kfsw_ftp_generate, 3, 0),
	SHELL_CMD_ARG(get, NULL, "Download: get <node> <remote> <local>.", cmd_kfsw_ftp_get, 4,
		      0),
	SHELL_CMD_ARG(list, NULL, "List directory: list <node> [path].", cmd_kfsw_ftp_list, 2,
		      1),
	SHELL_CMD_ARG(ls, NULL, "List directory: ls <node> [path].", cmd_kfsw_ftp_list, 2, 1),
	SHELL_CMD_ARG(mkdir, NULL, "Create directory: mkdir <node> <path>.",
		      cmd_kfsw_ftp_mkdir, 3, 0),
	SHELL_CMD_ARG(put, NULL, "Upload: put <node> <local> <remote>.", cmd_kfsw_ftp_put, 4,
		      0),
	SHELL_CMD_ARG(stat, NULL, "Show metadata: stat <node> <remote>.", cmd_kfsw_ftp_stat, 3,
		      0),
	SHELL_CMD_ARG(verify, NULL, "Compare local files: verify <first> <second>.",
		      cmd_kfsw_ftp_verify, 3, 0),
	SHELL_SUBCMD_SET_END);
