...
Housekeeping answers when asked and never speaks first, so something on the
ground has to do the asking. This is that: a CSP client over a KISS serial
link, small enough to read in one sitting, that requests samples and forwards
each one to a Yamcs UDP telemetry link.
